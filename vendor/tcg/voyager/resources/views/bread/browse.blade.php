                                        ...
                                        <tr>
                                            ...
                                            @foreach ($dataType->browseRows as $row)
                                                ...
                                                <td>
                                                    ...
                                                    {{-- my change --}}
                                                    <img alt="" title="" src="@if (!filter_var($data->{$row->field}, FILTER_VALIDATE_URL)) {{ Voyager::image($data->{$row->field}) }}@else{{ $data->{$row->field} }} @endif"
                                                        ...
                                                        @elseif ($data->{$row->field} != '')
                                                            ...
                                                            {{-- my change --}}
                                                                {{-- {{ $data->{$row->field} }} --}}
                                                                @php
                                                                    $filePath = $data->{$row->field}; // Assuming $data->{$row->field} contains the file path or URL
                                                                    $videoExtensions = ['mp4', 'avi', 'mov', 'wmv', 'mkv']; // Add more video extensions as needed
                                                                    $fileExtension = pathinfo($filePath, PATHINFO_EXTENSION);
                                                                    $isVideo = in_array($fileExtension, $videoExtensions);
                                                                @endphp
                                                                @if ($isVideo)
                                                                    <img src="https://img.uxwing.com/wp-content/themes/uxwing/download/video-photography-multimedia/video-icon.png" style="max-width:60px" alt="">
                                                        ...
                                                        @else
                                                            {{-- {{ trans_choice('voyager::media.files', 0) }} --}}
                                                            <span class="text-danger text-bold">EMPTY</span>
                                                        @endif
                                                    @else
                                                        @include('voyager::multilingual.input-hidden-bread-browse')
