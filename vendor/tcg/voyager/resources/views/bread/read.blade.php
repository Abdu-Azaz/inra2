                    ...
                    @foreach ($dataType->readRows as $row)
                        ...
                        <div class="panel-body" style="padding-top:0;">
                            ...
                            @elseif($row->type == 'media_picker')
                                @php
                                    $filePath =$dataTypeContent->{$row->field}; // Assuming $data->{$row->field} contains the file path or URL
                                ...
                                @endphp
                                @if ($isPdf)
                                    <a href="{{url('storage/'.$dataTypeContent->{$row->field})}}" target="_blank">PDF</a>
                                @elseif($isVideo)
                                    <video controls class="w-100" poster="{{ url('storage/' . $dataTypeContent->{$row->field}) }}">
                                        <source  src="{{ url('storage/' .$dataTypeContent->{$row->field}) }}">
                                    </video>
                                @else
                                
                                    <img  class="img-responsive " style="max-width:90px"
                                        src="{{ filter_var($dataTypeContent->{$row->field}, FILTER_VALIDATE_URL) ? $dataTypeContent->{$row->field} : Voyager::image($dataTypeContent->{$row->field}) }}"
                                        >
                                @endif
